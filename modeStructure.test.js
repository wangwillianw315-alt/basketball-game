...
async function assertLayupModeMovesPlayer() {
  await withPage({ width: 740, height: 360 }, async (page) => {
    await page.locator("#layupModeButton").click();
    await page.waitForTimeout(250);

    const before = await page.locator("#slingPlayer").boundingBox();
    await page.locator("#layupRightButton").dispatchEvent("pointerdown", { pointerId: 1 });
    await page.waitForTimeout(350);
    await page.locator("#layupRightButton").dispatchEvent("pointerup", { pointerId: 1 });
...
(async () => {
  ...
  await assertLayupModeMovesPlayer();
